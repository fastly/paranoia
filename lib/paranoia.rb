...
module Paranoia
  ...
  module Query
    ...
    def only_deleted
      with_deleted.where("#{table_name}.#{paranoia_column} IS NOT NULL")
    end
  ...
  def really_destroy!
    dependent_reflections = reflections.select do |_name, reflection|
  ...
  def destroyed?
    !!send(paranoia_column) # rubocop:disable Style/DoubleNegation
  end
  ...
  def restore_associated_records
    ...
    destroyed_associations.each do |association|
      ...
      next if association_data.nil?
      next unless association_data.paranoid?
...
class ActiveRecord::Base
  def self.acts_as_paranoid(options = {})
    ...
    default_scope { where(quoted_table_name + ".#{paranoia_column} IS NULL") }
